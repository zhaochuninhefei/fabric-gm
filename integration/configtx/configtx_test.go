...
var _ = Describe("ConfigTx", func() {
	...
	It("creates channels and updates them using fabric-config/configtx", func() {
		...
		envelope, err := configtx.NewEnvelope1(createChannelUpdate)
		Expect(err).NotTo(HaveOccurred())
		...
		signature, err := signingIdentity.CreateConfigSignature1(configUpdate)
		Expect(err).NotTo(HaveOccurred())

		By("creating a signed config update envelope with the orderer's detached signature")
		configUpdateEnvelope, err := configtx.NewEnvelope1(configUpdate, signature)
		Expect(err).NotTo(HaveOccurred())
		...
		configUpdateEnvelope, err = configtx.NewEnvelope1(configUpdate, signatures...)
		Expect(err).NotTo(HaveOccurred())
